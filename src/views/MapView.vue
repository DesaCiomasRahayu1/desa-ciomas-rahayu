  ...
  <div class="fixed">
    <l-map :zoom="zoom" :center="center" style="height: 100vh; width: 100vw">
      ...
      <l-geo-json
        ...
        :options="options"
        layer-type="overlay"
        :name="`${geojson.properties.penggunaan-lahan}`"
...
export default {
  ...
  data() {
    return {
      ...
      enableTooltip: true,
      zoom: 15,
      center: [-6.602577780908636, 106.76726595121995],
      layerData: [],
      geojson: null,
      fillColor: "#0CF9E0",
      tileProviders: tileProviders,
      marker: latLng(-6.602577780908636, 106.76726595121995),
      timeout: undefined,
    };
  },
  mounted() {
  ...
  methods: {
    async loadSomeGeoJson() {
      ...
      const response = await fetch("/batas-rw-sedesa-ciomas-rahayu.json");
  ...
  async created() {
    this.loading = true;
    const response = await fetch("/Landuse-penggunaan-lahan-desa-ciomas-rahayu");
    const data = await response.json();
    this.geojson = data;
    this.loading = false;
  },
